...
d = {
    "Name": pd.Series(["Tom", "James", "Ricky", "Vin", "Steve", "Smith", "Jack",
                       "Lee", "David", "Gasper", "Betina", "Andres"]),
    "Age": pd.Series([25, 26, 25, 23, 30, 29, 23, 25, 40, 23, 51, 23]),
    "Rating": pd.Series(
        [4.23, 3.24, 3.98, 2.56, 3.20, 4.6, 3.8, 3.78, 2.98, 4.80, 4.10, 3.65])
    }
df = pd.DataFrame(d)

temp = df.sort_values(["Age", "Name"], ascending=[True, False])
...
temp = df.groupby("Team")["Points"].sum()
# temp = grouped["Points"]["Year"]
...
temp = pd.merge(left, right, left_on="left_id", right_on="right_id")

noi = pd.concat([left, right])
